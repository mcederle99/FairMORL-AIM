...
class TD3(object):
    ...
    def select_action(self, state, edge_index, edge_attr, edge_type, omega):
        return self.actor(state, edge_index, edge_attr, edge_type, omega).cpu().data.numpy().flatten()
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            next_action = self.actor_target(batch.x_t, batch.edge_index_t, batch.edge_attr_t, batch.edge_type_t, batch.omega_t)
            noise = (
            ...
            target_q1, target_q2 = self.critic_target(batch, next_action, 't', batch.omega_t)
            target_q = torch.min(target_q1, target_q2)
            target_q = batch.reward.unsqueeze(dim=1) + batch.not_done.unsqueeze(dim=1) * self.discount * target_q
        # Get current q estimates
        current_q1, current_q2 = self.critic(batch, batch.actions, 's', batch.omega_s)
        # Compute critic loss
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            actor_loss = -self.critic.q1(batch, self.actor(batch.x_s, batch.edge_index_s,
                                                           batch.edge_attr_s, batch.edge_type_s, batch.omega_s), 's', batch.omega_s).mean()
